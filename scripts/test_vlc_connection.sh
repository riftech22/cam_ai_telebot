...
timeout 10 vlc -I dummy --rtsp-tcp --rtsp-frame-buffer-size=100000 "$RTSP_URL" vlc://quit 2>&1 | head -20 &
VLC_PID=$!
sleep 10
...
if id "vlc-user" &>/dev/null; then
    ...
    timeout 10 sudo -u vlc-user vlc -I dummy --rtsp-tcp --rtsp-frame-buffer-size=100000 "$RTSP_URL" vlc://quit 2>&1 | head -20 &
    VLC_PID=$!
    sleep 10
    kill $VLC_PID 2>/dev/null || true
    wait $VLC_PID 2>/dev/null || true
